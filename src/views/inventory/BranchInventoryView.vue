...
export default {
  ...
  methods: {
    ...
    async fetchBranchList() {
      try {
        const response = await axios.get('/inventory/branches');
    ...
    async fetchInventoryData() {
      ...
      try {
        const response = await axios.get(`/inventory/branch/${this.selectedBranchId}`);
    ...
    async openRequestModal(item) {
      ...
      try {
        ...
        const response = await axios.post('/inventory/request/manual', params);
