            ...
            <div class="order-body" style="flex-direction: column; align-items: flex-start; gap: 4px;">
              ...
              <div v-if="order.expanded" class="order-detail-box" @click.stop>
                ...
                <div v-for="(item, idx) in order.items" :key="idx" class="detail-item-row">
                  ...
                  <!-- 💡 음료 및 상품 옵션 정보 출력 -->
                  <div v-if="item.options" class="option-tag">
                    옵션: {{ formatOptions(item.options) }}
                  </div>
                ...
                <div style="margin-top: 8px; font-size: 12px; color: #64748b; border-top: 1px dashed #e2e8f0; padding-top: 6px;">
      ...
      <div class="receipt-modal-content" @click.stop>
        ...
        <div class="receipt-items-list">
          <div v-for="(item, idx) in selectedReceipt?.items" :key="idx" class="receipt-item-row">
            ...
            <!-- 영수증 내부 옵션 정보 출력 -->
            <div v-if="item.options" class="receipt-sub-tag">
              옵션: {{ formatOptions(item.options) }}
            </div>
...
const getItemDisplayName = (item) => {
  ...
  if (Array.isArray(rawFlavors) && rawFlavors.length > 0) {
    const m = rawFlavors[0];
    firstFlavor = typeof m === 'object' && m !== null ? (m.name || m.flavorName || '') : m;
  } else if (typeof rawFlavors === 'string') {
    firstFlavor = rawFlavors;
  }

  if (!baseName || baseName === '기본 메뉴') {
    if (item.sizeName && icecreamSizes.some(s => item.sizeName.includes(s))) {
      return '아이스크림';
    }
    if (firstFlavor) {
      return firstFlavor;
...
const formatItemFlavors = (item) => {
  ...
  const icecreamSizes = ['파인트', '쿼터', '패밀리', '하프갤런', '싱글', '더블', '싱글콘', '더블콘', '싱글컵', '더블컵'];
  const isIceCreamSize = item.sizeName && icecreamSizes.some(s => item.sizeName.includes(s));

  if (isIceCreamSize || flavorList.length > 1) {
    return flavorList.join(', ');
  }

  const displayName = getItemDisplayName(item);
  if (flavorList.length === 1 && flavorList[0] !== displayName) {
    return flavorList[0];
  }
