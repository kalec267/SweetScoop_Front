  ...
  <div class="dashboard-container">
    ...
    <div class="dashboard-header">
      <div class="title-area">
        ...
        <!-- 권한별 드롭다운 (지점 관리자는 본인 매장 고정/비활성화) -->
        <select 
          ...
          class="branch-select"
        >
          <option v-if="userRole === 'HQ'" :value="0">전체 지점 통합 통계</option>
          <option v-for="b in branchList" :key="b.id" :value="b.id">
            {{ b.id }}번 지점 ({{ b.branchName }})
          </option>
        </select>
        
        <span v-if="userRole === 'BRANCH'" class="role-badge">지점 관리자</span>
    ...
    <div class="main-layout">
      ...
      <!-- [섹션 4] 최하단: 지점별 통계 표 (본사가 전체 통계를 볼 때만 노출) -->
      <div v-if="userRole === 'HQ' && selectedBranchId === 0" class="branch-ranking-section" style="margin-top: 24px;">
        <h3>지점별 매출 통계 랭킹</h3>
...
export default {
  ...
  async mounted() {
    ...
    // 권한별 기본 selectedBranchId 설정
    if (this.userRole === 'BRANCH') {
      this.selectedBranchId = this.myBranchId; // 지점 관리자: 내 지점 고정
    } else {
  ...
  methods: {
    ...
    async fetchBranchList() {
      try {
        const response = await axios.get('/api/inventory/branches');
...
.branch-select { padding: 8px 12px; font-size: 14px; border: 1px solid #cbd5e1; border-radius: 6px; background-color: #fff; font-weight: 600; color: #334155; outline: none; cursor: pointer; }
.branch-select:disabled { background-color: #f1f5f9; cursor: not-allowed; }
