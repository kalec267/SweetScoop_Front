...
<script setup="setup">
    ...
    const registerForm = ref({loginId: '', password: '', name: '', branchId: null});

    const loginForm = ref({role: 'HQ', username: '', password: ''});
    ...
    const handleRegister = async () => {
        try {
            ...
            const response = await axios.post('/api/admin/auth/register', {
                ...
                branchId: registerForm.value.branchId
                    ? Number(registerForm.value.branchId)
                    : null
            });
