...
<tbody>
  <tr v-if="paginatedNotice.length === 0">
...
<div v-if="isDetailModalOpen" class="modal-overlay">
  <div class="modal-content">
    ...
    <div class="form-group">
      <label>작성자</label>
      <p>{{ selectedNotice.hqManagerName ?? '-' }}</p>
    </div>    
...
const submitNotice = async () => {
  ...
  if(isEditMode.value){
    ...
    await axios.put(
      `http://localhost:8888/notice/${selectedNoticeId.value}`,
      noticeForm.value
    )
...
const removeNotice = async (id) => {
  ...
  await axios.delete(`http://localhost:8888/notice/${id}`)
