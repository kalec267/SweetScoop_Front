  ...
  <tbody>
    ...
    >
      ...
      <td>
        ...
        <button
        class="btn-detail"
        @click="openDetailModal(cs)">
        상세보기
      </button>
    <button
    v-if="userRole === 'BRANCH' && !cs.answer"
    class="btn-edit"
  ...
  >
    ...
    v-if="csList.length > 0"
  >
    <div class="pagination-info">
      전체 {{ csList.length }}개 항목 중
  ...
  const submitCS = async () => {
    ...
    if (isEditMode.value) {
      ...
      await axios.put(
        `http://localhost:8888/cs/${selectedCSId.value}`,
        csForm.value
      )
...
//수정할 값
  ...
  const removeCS = async (id) => {
    ...
    await axios.delete(`http://localhost:8888/cs/${id}`)
  ...
  const submitAnswer = async () => {
    ...
    await axios.put(
      `http://localhost:8888/cs/${selectedCS.value.id}/answer`,
      {
